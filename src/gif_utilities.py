...
from src.aux import noop_image


def get_total_duration_milliseconds(image: Image):
...
def cut_gif(img: Image, start: int, end: int = None, watermark=noop_image):
    gif = unpack_gif(img)
    gif_duration = get_total_duration_milliseconds(img)
    start = max(start, 0)
